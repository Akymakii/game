...
def draw_bonus_timer():
    ...
    if speed_boost_active:
        timer_text = font.render(f"УСКР: {speed_boost_duration // 60}", True,
                                 RED)  # Показываем оставшееся время в секундах
...
def draw_player(direction):
    ...
    if shield_active:
        pygame.draw.circle(screen, (0, 191, 255),
                           (player_pos[0] + player_size // 2 + 15, player_pos[1] + player_size // 2 + 15),
                           player_size * 1.5, 3)
...
def game_loop():
    global player_pos, score, enemies, current_frame, frame_counter, enemy_speed, enemy_spawn_rate, lives, bonuses, \
        player_speed, shield_active, game_paused
    player_pos = [WIDTH // 2, HEIGHT // 2]
    score = 0
    lives = 3
    enemies.clear()
